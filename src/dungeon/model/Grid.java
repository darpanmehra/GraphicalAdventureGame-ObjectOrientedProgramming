...
class Grid {
  ...
  protected Grid(int numRow, int numCol, int interConnectivity, String dungeonType,
    ...
    //Assign thief to the dungeon as the same number of monsters
    assignThieves(monsterCount);

    //Assign pits to the dungeon as the same number of monsters
    assignPits(monsterCount);
  }
  ...
  private void assignThieves(int thiefCount) {
    while (thiefCount > 0) {
      int randomRow = random.nextInt(maze.length);
      int randomCol = random.nextInt(maze[0].length);
      ILocation randomLocation = maze[randomRow][randomCol];
      if (randomLocation.equals(startLocation) || randomLocation.equals(endLocation)
              || randomLocation.hasThief()) {
        continue;
      }
      randomLocation.setThief();
      thiefCount--;
    }
  }

  private void assignPits(int pitCount) {
    while (pitCount > 0) {
      int randomRow = random.nextInt(maze.length);
      int randomCol = random.nextInt(maze[0].length);
      ILocation randomLocation = maze[randomRow][randomCol];
      if (randomLocation.equals(startLocation) || randomLocation.equals(endLocation)
              || randomLocation.hasPit() || randomLocation.hasMonster()) {
        continue;
      }
      randomLocation.setPit();
      pitCount--;
    }
  }
