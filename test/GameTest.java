...
public class GameTest {
  ...
  @Test
  public void testThiefCount() {
    model = new GameState(6, 6, interconnectivity,
            "nonwrapping", 20, 5, random);

    ILocation[][] dungeon = model.getDungeon();
    int totalLocationHasThief = 0;
    for (int i = 0; i < dungeon.length; i++) {
      for (int j = 0; j < dungeon[i].length; j++) {
        if (dungeon[i][j].hasThief()) {
          totalLocationHasThief = totalLocationHasThief + 1;
        }
      }
    }
    assertEquals(5, totalLocationHasThief);
  }

  @Test
  public void testPitsCount() {
    model = new GameState(6, 6, interconnectivity,
            "nonwrapping", 20, 5, random);

    ILocation[][] dungeon = model.getDungeon();
    int totalLocationHasPit = 0;
    for (int i = 0; i < dungeon.length; i++) {
      for (int j = 0; j < dungeon[i].length; j++) {
        if (dungeon[i][j].hasPit()) {
          totalLocationHasPit = totalLocationHasPit + 1;
        }
      }
    }
    assertEquals(5, totalLocationHasPit);
  }
  ...
  @Test
  public void testFallingIntoPitAndGameEnd() {
    model = new GameState(6, 6, 100,
            "nonwrapping", 20, 5, random);

    //Move player to pit location
    model.movePlayer(Direction.WEST);
    model.movePlayer(Direction.WEST);

    assertEquals(true, model.isGameOver());
    assertEquals("You died by falling into a pit. Better luck next time.\n",
            model.getGameOverStatus());
  }
