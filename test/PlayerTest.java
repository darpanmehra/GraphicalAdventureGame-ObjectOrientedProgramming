...
public class PlayerTest {
  ...
  public void testSteal() {
    ...
    //Player enters location with thief
    ILocation thiefLocation = new Location(0, 0, rand);
    thiefLocation.setThief();
    player.setCurrentLocation(thiefLocation);
    assertEquals("{}", player.getTreasures().toString()); // loses all treasures
  }

  @Test
  public void testPlayerFallingIntoPitAndKilled() {
    ILocation location = new Location(0, 0, random);
    ILocation south = new Location(1, 0, random);
    location.joinLocationToSouthDirection(south);
    location.setPit();
    ...
    player.setCurrentLocation(location); // Set location with pit
    assertEquals(player.getHealth(), 0);
    assertFalse(player.isAlive());
  }
}